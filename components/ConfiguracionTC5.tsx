...
export default function WebViewConfiguracionTC5() {
    ...
    /* -------- Obtener portal URL (prod/dev) al montar ---------- */
    useEffect(() => {
        (async () => {
    ...
    if (hasError) {
        return (
            <SafeAreaView style={styles.container}>
                <View style={styles.errorContainer}>
                    {/* … UI de error igual que antes … */}
                </View>
            </SafeAreaView>
        );
    }

    /* ---- Esperar a que baseUrl esté lista antes de continuar --- */
    if (!baseUrl) {
        return (
            <SafeAreaView style={styles.container}>
    ...
    /* ------------- Construir URL de configuración -------------- */
    const queryParams =
        `mac=${mac}&token=${token}&idioma=${idioma}` +
        `&idNave=${idSite}&app=appmovilv3&type=control-remoto`;

    const url = `${baseUrl}?${queryParams}`;
    console.log("🎛️ Configuración URL:", url);
