...
// type WebViewRouteProp = RouteProp<RootStackParamList, "HistoriaAlarmas">;
...
//     const queryParams = `mac=${mac}&token=${token}&idioma=${idioma}&nave=${siteName}&idNave=${buildingPortalRef}&app=appmovilv3&type=historico_alarmas`;
...
export default function WebViewExplotacion() {
    ...
    /* ---------------------------------------------------- *
     * 1. Obtener URL del portal (prod o dev) una sola vez  *
     * ---------------------------------------------------- */
    useEffect(() => {
        (async () => {
            const url = await getPortalUrl();   // usa el helper (lee isDeveloperMode)
            setBaseUrl(url);
        })();
    }, []);
    ...
    /* ------------ Pantalla de error HTTP / WebView -------- */
    if (hasError) {
        return (
            <SafeAreaView style={styles.container}>
                <View style={styles.errorContainer}>
    ...
    const queryParams =
        ...
        `&app=appmovilv3&type=historico_alarmas`;

    const url = `${baseUrl}?${queryParams}`;

    console.log("🔗 Explotación URL:", url);
